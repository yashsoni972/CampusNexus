  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 stagger">
        ...
        <StatCard icon={MegaphoneIcon}          label="Announcements"    value={overview.totalAnnouncements} sub={`${overview.announcementsThisMonth} this month`}     color="bg-cyan-100 text-cyan-600"      href="/announcements" />
      ...
      <div className="card overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-50">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5 ml-8">Click ✏️ edit icon to update any student's attendance</p>
          </div>
          <button
        ...
        <div className="px-5 py-3 border-b border-gray-50 bg-gray-50/50">
          <div className="relative max-w-xs">
            ...
            <input
              ...
              value={searchStudent}
              onChange={(e) => { setSearchStudent(e.target.value); fetchStudents(e.target.value); }}
            ...
            <div className="p-5 space-y-4">
              ...
              {attendanceValue !== '' && (
                <div className={`text-xs font-semibold px-3 py-2 rounded-lg ${
                  ...
                  {parseFloat(attendanceValue) >= 75 ? '✅ Good standing' : parseFloat(attendanceValue) >= 60 ? '⚠️ Borderline' : '❌ Below minimum'}
                </div>
              )}
    ...
    <div className="space-y-5">
      ...
      <div className="card p-5">
        ...
        <p className={`text-xs font-semibold mt-3 px-3 py-1.5 rounded-lg inline-block ${attendance >= 75 ? 'bg-emerald-50 text-emerald-700' : attendance >= 60 ? 'bg-amber-50 text-amber-700' : 'bg-red-50 text-red-700'}`}>
          {attendance >= 75 ? '✅ Your attendance is in good standing' : attendance >= 60 ? '⚠️ Attendance is borderline — improve it to stay eligible' : '❌ Attendance is below minimum requirement. Please attend more classes.'}
        </p>
      </div>
              ...
              <Link to={`/announcements/${a._id}`} key={a._id} className="flex items-start gap-3 px-5 py-3.5 hover:bg-gray-50 transition-colors">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    {a.isPinned && <span className="text-amber-500 text-xs">📌</span>}
                    <p className="text-sm font-semibold text-gray-900 truncate">{a.title}</p>
                  </div>
                  <p className="text-xs text-gray-400 mt-0.5">{timeAgo(a.createdAt)}</p>
                </div>
